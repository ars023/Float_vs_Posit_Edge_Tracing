...
from softposit import quire16, posit16
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D

def plane(qx, qy, qz, sx, sy, sz, tx, ty, tz):
    # Convert each coordinate individually to posit16
    qx, qy, qz = sp.posit16(qx), sp.posit16(qy), sp.posit16(qz)
    sx, sy, sz = sp.posit16(sx), sp.posit16(sy), sp.posit16(sz)
    tx, ty, tz = sp.posit16(tx), sp.posit16(ty), sp.posit16(tz)
...
def intersect(px, py, pz, rx, ry, rz, z1, z2, z3, z4):
    # Helper function to safely initialize posit16
    def safe_posit16(value):
        if not isinstance(value, sp.posit16):
            return sp.posit16(value)
        return value

    # Convert inputs to safe posit16
    px, py, pz = safe_posit16(px), safe_posit16(py), safe_posit16(pz)
    rx, ry, rz = safe_posit16(rx), safe_posit16(ry), safe_posit16(rz)
    z1, z2, z3, z4 = safe_posit16(z1), safe_posit16(z2), safe_posit16(z3), safe_posit16(z4)
    ...
    h = z4 - q_h.toPosit()  # Subtract from z4 and convert to posit16
    ...
    denom = q_denom.toPosit()  # Convert to posit16
    ...
    q_ix.qma(px, sp.posit16(1))  # Add px
    ix = q_ix.toPosit()  # Convert to posit16

    q_iy = quire16()
    q_iy.qma(i, ry)  # Add i * ry
    q_iy.qma(py, sp.posit16(1))  # Add py
    iy = q_iy.toPosit()  # Convert to posit16

    q_iz = quire16()
    q_iz.qma(i, rz)  # Add i * rz
    q_iz.qma(pz, sp.posit16(1))  # Add pz
    iz = q_iz.toPosit()  # Convert to posit16
...
def cross_product_quire(a, b):
    ...
    q_cx.qma(a[2], posit16(-1) * b[1])  # Subtract a_z * b_y from the quire

    # Compute c_y = a_z * b_x - a_x * b_z
    q_cy.qma(a[2], b[0])  # Add a_z * b_x to the quire
    q_cy.qma(a[0], posit16(-1) * b[2])  # Subtract a_x * b_z from the quire

    # Compute c_z = a_x * b_y - a_y * b_x
    q_cz.qma(a[0], b[1])  # Add a_x * b_y to the quire
    q_cz.qma(a[1], posit16(-1) * b[0])  # Subtract a_y * b_x from the quire

    # Convert quires back to posit16 for the result
    c_x = q_cx.toPosit()
    c_y = q_cy.toPosit()
    c_z = q_cz.toPosit()
...
ax.set_title('Ray and Triangle Intersections with Posit16')
